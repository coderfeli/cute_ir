// Test cute.composition operation
func.func @test_composition() -> index {
  ...
  %shape_b = cute.make_shape %c8, %c16 : (index, index) -> !cute.shape<2>
  %stride_b = cute.make_stride %c2, %c16 : (index, index) -> !cute.stride<2>
...
func.func @test_composition_crd2idx() -> index {
  ...
  %shape_a = cute.make_shape %c4, %c8 : (index, index) -> !cute.shape<2>
  %stride_a = cute.make_stride %c1, %c4 : (index, index) -> !cute.stride<2>
  %layout_a = cute.make_layout %shape_a, %stride_a : (!cute.shape<2>, !cute.stride<2>) -> !cute.layout<2>
  
  // Layout B: with stride multipliers
  %shape_b = cute.make_shape %c4, %c8 : (index, index) -> !cute.shape<2>
  ...
  %coord = cute.make_coord %c1, %c2 : (index, index) -> !cute.coord<2>
  %idx = cute.crd2idx %coord, %composed : (!cute.coord<2>, !cute.layout<2>) -> index
...
func.func @test_composition_shape() {
  ...
  %result_shape = cute.get_shape %composed : !cute.layout<2> -> !cute.shape<2>
  %rank = cute.rank %result_shape : !cute.shape<2> -> index
