...
module attributes {
  cute.target_arch = "gfx942",
  cute.target_vendor = "amd"
} {
  
  // Test basic layout creation for AMD
  func.func @test_make_layout_amd() -> !cute.layout<2> {
    // Create shape and stride for MFMA-friendly layout
    // GFX942 MFMA prefers 32x32 or 16x16 tiles
    %shape = cute.make_shape 32, 32 : !cute.shape<2>
    %stride = cute.make_stride 1, 32 : !cute.stride<2>
  ...
  func.func @test_layout_size_amd(%arg0: !cute.layout<2>) -> !cute.int {
  ...
  func.func @test_coord_to_index_amd() {
    ...
    %coord = cute.make_coord 8, 16 : !cute.coord<2>
  ...
  func.func @test_mfma_16x16x16_layout() -> !cute.layout<3> {
    // MFMA f32_16x16x16_f16 layout
    %shape = cute.make_shape 16, 16, 16 : !cute.shape<3>
    %stride = cute.make_stride 1, 16, 256 : !cute.stride<3>
  ...
  func.func @test_wavefront_partitioned_layout() -> !cute.layout<2> {
  ...
  func.func @test_tiled_mfma_layout() -> !cute.layout<2> {
    // Outer tile: 4x4 MFMA atoms
    // Inner tile: 32x32 MFMA shape
    // Total: 128x128
    %shape = cute.make_shape 128, 128 : !cute.shape<2>
    %stride = cute.make_stride 1, 128 : !cute.stride<2>
    %layout = cute.make_layout %shape, %stride : !cute.layout<2>
  ...
  func.func @test_coalesced_global_layout() -> !cute.layout<2> {
    // Layout ensuring 128-byte alignment
    // 32 threads × 4 elements (f32) = 128 bytes
    %shape = cute.make_shape 32, 1024 : !cute.shape<2>
    %stride = cute.make_stride 1, 32 : !cute.stride<2>
    %layout = cute.make_layout %shape, %stride : !cute.layout<2>
  ...
  func.func @test_gemm_layout_gfx942() {
    ...
    %shape_a = cute.make_shape 128, 32 : !cute.shape<2>
    %stride_a = cute.make_stride 1, 128 : !cute.stride<2>
    %layout_a = cute.make_layout %shape_a, %stride_a : !cute.layout<2>
    
    // B matrix layout (K x N)
    %shape_b = cute.make_shape 32, 128 : !cute.shape<2>
    %stride_b = cute.make_stride 1, 32 : !cute.stride<2>
    %layout_b = cute.make_layout %shape_b, %stride_b : !cute.layout<2>
    
    // C matrix layout (M x N)
    %shape_c = cute.make_shape 128, 128 : !cute.shape<2>
    %stride_c = cute.make_stride 1, 128 : !cute.stride<2>
    %layout_c = cute.make_layout %shape_c, %stride_c : !cute.layout<2>
  ...
  func.func @test_mfma_bf16_layout() -> !cute.layout<3> {
    // MFMA f32_32x32x16_bf16 layout
    %shape = cute.make_shape 32, 32, 16 : !cute.shape<3>
    %stride = cute.make_stride 1, 32, 1024 : !cute.stride<3>
    %layout = cute.make_layout %shape, %stride : !cute.layout<3>
  ...
  func.func @test_mfma_f64_layout() -> !cute.layout<3> {
    // MFMA f64_16x16x4_f64 layout
    %shape = cute.make_shape 16, 16, 4 : !cute.shape<3>
    %stride = cute.make_stride 1, 16, 256 : !cute.stride<3>
    %layout = cute.make_layout %shape, %stride : !cute.layout<3>
