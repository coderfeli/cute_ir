...
def benchmark_matrix_transpose(TILE_SIZE=8, BLOCK_TILE=32):
    ...
    print("Size: 4096x4096 matrix (16M floats, ~64 MB)")
    print("Memory Traffic: 2 x 4096 x 4096 x 4 bytes = ~134 MB per kernel")
    print(f"TILE_SIZE: Each thread processes {TILE_SIZE} elements ({TILE_SIZE//2} vec2)")
    print(f"BLOCK_TILE: {BLOCK_TILE}x{BLOCK_TILE}")
    print("="*80)
...
def test_benchmark_matrix_transpose():
    ...
    print("="*80)
    # Test with TILE_SIZE=8, BLOCK_TILE=32
    assert benchmark_matrix_transpose(TILE_SIZE=8, BLOCK_TILE=32), "Matrix transpose benchmark failed correctness check"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tile-size', type=int, default=8,
                       help='Elements per thread (default: 8)')
    ...
    if result:
        ...
        print("BENCHMARK FAILED CORRECTNESS CHECK")
        sys.exit(1)
